...
#UI制御用のグローバル変数群
application = wx.App()
frame = wx.Frame(None, wx.ID_ANY, '商圏探索機', size=(600, 400))

panel = wx.Panel(frame, wx.ID_ANY,size=(300,400))
panel.SetBackgroundColour('#AFAFAF')

text_1 = wx.TextCtrl(panel, wx.ID_ANY,"1600021")
text_2 = wx.TextCtrl(panel, wx.ID_ANY,"./data/test.csv")
text_5 = wx.TextCtrl(panel, wx.ID_ANY,"80")
...
def tam(event):
    ...
    shop_adress = adress_manager_v2(x,"",master)#特殊仕様
    ...
    text_3.SetValue("80%商圏は"+str(round(ans/1000, 3))+'kmです。')
    #print("人数は",amount,"人で、全体の",round(amount/cc, 3)*100,"%です。")
    text = "郵便番号\t商品\t\t人数"
    for i in range(len(c_ans)):
        text += "\n"+str(c_ans.iloc[i,0])+"\t"+str(c_ans.iloc[i,2])+"\t"+str(c_ans.iloc[i,3])+"人"
    button_2.Enable()
    button_3.Enable()
    button_5.Enable()
    text_4.SetValue(text)
    meter = round(ans/1000, 3)
    m = folium.Map(location=[shop_adress[0], shop_adress[1]], zoom_start=11)
    folium.Marker([shop_adress[0], shop_adress[1]], popup=x+"\n"+str(meter)+"km", icon=folium.Icon(color='blue')).add_to(m)
    folium.Circle(
    ...
    m.save("map.html")
    tmpurl='file:///C:/Users/yuya_kiyama/python/Yubin/pg/map.html'.format(path=os.getcwd(),mapfile="map.html")
    browser = webdriver.Chrome()
    browser.set_window_size(600,1200)
    browser.get(tmpurl)
    ...
    bitmap = wx.BitmapButton(p, wx.ID_ANY, wx.Bitmap("map.png"), size=(300, 600), style=wx.SIMPLE_BORDER)
    layout4.Add(bitmap,proportion=1)
    p.SetSizer(layout4)
    layout5 = wx.BoxSizer(wx.HORIZONTAL)
    layout5.Add(panel,proportion=1)
    layout5.Add(p,proportion=1)
    frame.SetSizer(layout5)
    frame.Show()

def save(event):
    global cnt
    temp = text_1.GetValue()
    df = client_count
    df.to_csv(str(temp)+"-list"+str(cnt)+".csv",index=False,encoding="cp932")
    cnt += 1
    button_2.Disable()
...
def savemap(event):
    ...
    if cnt2 != cnt-1:
        cnt2 = cnt-1
    m.save(str(temp)+"-map"+str(cnt2)+".html")
    button_5.Disable()

def map_window(event):
    global browser
    tmpurl='file:///C:/Users/yuya_kiyama/python/Yubin/pg/map.html'.format(path=os.getcwd(),mapfile="map.html")
    browser = webdriver.Chrome()
...
def main():
    ...
    button_3.Bind(wx.EVT_BUTTON, map_window)
    ...
    p = wx.Panel(frame, wx.ID_ANY)
    bitmap = wx.BitmapButton(p, wx.ID_ANY, wx.Bitmap("back.png"), size=(300, 600), style=wx.SIMPLE_BORDER)
    layout2.Add(bitmap,proportion=1)
    p.SetSizer(layout2)

    layout3 = wx.BoxSizer(wx.HORIZONTAL)
    layout3.Add(panel,proportion=1)
    layout3.Add(p,proportion=1)
    frame.SetSizer(layout3)
